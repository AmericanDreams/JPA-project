...
<div class="container">
	...
	<form method="post" action="<%=request.getContextPath() %>/EditPilot">

		<div class="form-group ${ (nameError!= null) ? 'has-error': '' }">
			<label for="exampleInputEmail1">${addPilot_label_name }</label> <input
				type="text" class="form-control " id="name" name="name"
				placeholder="Name" value='<c:out value='${name }'/>'> <small
				class="text-danger"> ${nameError } </small>
		</div>
		<div class="form-group ${ (surnameError!= null) ? 'has-error': '' }">
			<label for="exampleInputEmail1">${addPilot_label_surname }</label> <input
				type="text" class="form-control" id="surname" name="surname"
				placeholder="Surname" value="<c:out value='${surname }'/>">
			<small class="text-danger"> ${surnameError } </small>
		</div>
		...
		<div class="form-group ${ (pilotRankError!= null) ? 'has-error': '' }">
			<label for="exampleInputEmail1">${addPilot_label_pilotRank }</label>
			<select class="form-control" name="pilotRank">
				<c:forEach items="${pilotRanks}" var="rank">

					<c:choose>
						<c:when test="${rank == pilotRank }">
			...
			</select> <small class="text-danger"> ${pilotRankError } </small>
		</div>


		<input type="hidden" value="${pilotId }" name="pilotId" />
		<button type="submit" class="btn btn-default">Edit Pilot</button>
